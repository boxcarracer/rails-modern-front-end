class TicketOrdersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @ticket_order.save
        format.html { redirect_to ticket_order_url(@ticket_order), notice: "Ticket order was successfully created." }
  ...
  def update
    respond_to do |format|
      if @ticket_order.update(ticket_order_params)
        format.html { redirect_to ticket_order_url(@ticket_order), notice: "Ticket order was successfully updated." }
  ...
  def ticket_order_params
    params.require(:ticket_order).permit(:concert_id, :status, :count)
  end
end
